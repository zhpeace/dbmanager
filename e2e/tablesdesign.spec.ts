...
test('design table shows columns from schema cache', async ({ page }) => {
  ...
  const dlg = page.locator('[role="tabpanel"][data-state="active"]')
  await expect(dlg.getByRole('columnheader', { name: 'Type' })).toBeVisible({ timeout: 10_000 })
...
test('design table adds a column', async ({ page }) => {
  ...
  // 3 existing rows * 5 inputs = 15, new row name is input #15 (before Apply reloads schema)
...
test('design table indexes tab adds an index', async ({ page }) => {
  ...
  // index add form: name (placeholder idx_users_) + column picker popover (checkbox list)
