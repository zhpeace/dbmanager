...
- **Region copy/paste (DataGrip-aligned)**: click a cell to set the anchor, then Shift+click (or drag) another cell to select a rectangular region — the whole region highlights. Cmd/Ctrl+C copies the region as Excel-style TSV (rows newline-separated, columns tab-separated, so a block copied from a spreadsheet pastes straight in); Cmd/Ctrl+V parses the clipboard and fills the grid cell-by-cell from the anchor, each changed cell entering the dirty buffer and submitting via Save; a *plain-text* (single-value) clipboard instead fills the whole selected region with that value (Navicat-style), while a multi-cell clipboard aligns cell-by-cell. Out-of-range cells are truncated, empty clipboard cells are skipped (they never clear a cell), and deleted rows are ignored. Single-cell selection keeps the previous cell copy/paste behavior unchanged
