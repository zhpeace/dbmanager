...
- **Sidebar database sync**: clicking a database in the sidebar updates the tab's per-connection database state and the top strip immediately (previously the strip stayed on "no database" until a database was picked in the editor); the strip always shows the active connection with its current database, while the editor selector keeps following the tab's bound connection. Clicking a database of a *different* connection activates that connection first (matching the table-click path), and a new query tab inherits the active connection and its currently selected database (Navicat/DBeaver-style). A *single* click on an object (table/view/... ) also switches the top strip to its connection and database, not only double-click; for PostgreSQL-style schema trees the object's database (not the schema name) is used
- **Schema Designer**: Visual table creation and modification (add/drop/modify/rename columns)
- **Data Browser**: Paginated table browsing with scroll-based lazy loading; single-clicking a cell selects (highlights) it, Cmd/Ctrl+C copies the selected cell, and Cmd/Ctrl+V pastes the clipboard into it by entering edit mode (DataGrip/Navicat-style); inside the inline editor the system copy/paste works natively with stable cursor behavior. A single sidebar click on an object only switches the top strip to its connection/database and never clears the currently browsed table
