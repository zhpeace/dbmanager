...
pub fn db_kind(conn: &DbConnection) -> Result<DbKind, String> {
    match conn {
        ...
        #[cfg(feature = "dameng")]
        DbConnection::Dameng(_) => Ok(DbKind::Oracle),
...
fn col_type_for(kind: &DbKind, col: &ColumnDef) -> String {
    ...
    match kind {
        DbKind::Oracle => {
            // Pass through types already written in Oracle/Dameng native syntax
            // (e.g. NUMBER(10,2), VARCHAR2(50), CHAR(10), DATE, TIMESTAMP(6),
            // CLOB, BLOB, RAW, INTERVAL ...) so they are not mangled.
            if t.starts_with("NUMBER")
                || t.starts_with("VARCHAR2")
                || t.starts_with("NVARCHAR2")
                || t.starts_with("NCHAR")
                || t == "CHAR"
                || t.starts_with("CHAR(")
            ...
            if t.contains("INT") {
                "NUMBER(38)".to_string()
            } else if t.contains("VARCHAR") || t.contains("TEXT") {
                if t.contains("CHAR") {
                    let len = extract_len(&col.data_type, 255);
...
fn build_column(kind: &DbKind, col: &ColumnDef) -> String {
    ...
    let t = col.data_type.to_uppercase();
    let oracle_identity = matches!(kind, DbKind::Oracle)
        && col.primary_key
        && (t.contains("INT") || t.starts_with("NUMBER"));
    if oracle_identity {
        // Identity columns are implicitly NOT NULL and cannot carry a DEFAULT.
        s.push_str(" NOT NULL");
    ...
    } else if col.primary_key && matches!(kind, DbKind::MySql | DbKind::Sqlite) {
...
pub fn add_column_sql(kind: &DbKind, schema: Option<&str>, table: &str, col: &ColumnDef) -> String {
    let q = quote_schema_table(kind, schema, table);
    if matches!(kind, DbKind::Oracle) {
        // Oracle / Dameng do not accept the `COLUMN` keyword after ADD.
        format!("ALTER TABLE {} ADD ({})", q, build_column(kind, col))
    } else {
