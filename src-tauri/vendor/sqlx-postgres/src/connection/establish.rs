            ...
            match message.format {
                BackendMessageFormat::Authentication => match message.decode()? {
                    ...
                    Authentication::Sha256Password(body) => {
                        // openGauss-specific sha256 authentication. The response is an
                        // ASCII hex string, sent in a [PasswordMessage].
                        log::debug!(
                            "auth: openGauss sha256 (iteration={:?}, signature={})",
                            body.server_iteration(),
                            body.server_signature().is_some()
                        );

                        let password = options.password.as_deref().unwrap_or_default();
                        let response =
                            crate::connection::sha256::rfc5802_algorithm(password.as_bytes(), &body)?;

                        let response = std::str::from_utf8(&response).map_err(Error::protocol)?;

                        stream.send(Password::Cleartext(response)).await?;
                    }
